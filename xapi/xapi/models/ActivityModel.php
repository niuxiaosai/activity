...
class ActivityModel extends BaseModel {
	...
	public function save() {
		...
		foreach ( $array_request as $key => $value ) {
			...
			if ($value == 'hit_count' || $value == 'status' || $value == 'win_count') {
		...
		// 验证是否有重复数据
		$alias = sha1 ( $title );
		$repeat = $this->getRepeat ( $alias, NULL );
		if ($repeat) {
		...
		$planId = $this->saveBaseInfo ( $data );
		if ($planId > 0) {
			// 插入时段从表数据
			$res = $this->saveActivityTime ( $interval_info, $planId );
			if ($res == false) {
				// 删掉主表数据
				\core\ActivityDao::GetInstance ()->Del ( $planId );
				$response->status = ErrorMap::E_ACTIVITY_TIME_INSERT_ERROR;
				$response->msg = ErrorMap::GetErrorMsg ( $response->status );
				return $response;
			}
			// 插入奖品从表数据
			$res1 = $this->saveActivityPrize ( $activity_prize, $planId );
			if ($res1 == false) {
				// 删掉主表数据
				\core\ActivityDao::GetInstance ()->Del ( $planId );
				$response->status = ErrorMap::E_ACTIVITY_PRIZE_INSERT_ERROR;
				$response->msg = ErrorMap::GetErrorMsg ( $response->status );
				return $response;
			}
		} else {
			$response->status = ErrorMap::E_ACTIVITY_INSERT_ERROR;
			$response->msg = ErrorMap::GetErrorMsg ( $response->status );
			return $response;
		}
		return $response;
