<?php
/*
 * creator: hexuan, ericfu
 * */

set_time_limit(0);
declare(ticks = 1); //没执行一个opcode都去检查中断
...
abstract class TcpServer
{
  ...
  public function __construct($max_length = 10240, $black_list = false, $white_list = false, $back_log = 50, $wrap = true)
  {
  ...
  private function init()
  {
    ...
    $this->server_handle = @socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
    if ($this->server_handle === false)
    {
      $err = @socket_strerror(@socket_last_error());
      Log::Error("socket create fail.{$err}");
      exit("error: socket create fail.{$err}\n");
    }

    if (!@socket_set_option($this->server_handle, SOL_SOCKET, SO_REUSEADDR, 1))
    {
      $err = @socket_strerror(@socket_last_error());
      @socket_close($this->server_handle);
      Log::Error($err);
      exit($err);
    }

    $bind = @socket_bind($this->server_handle, $this->ip, $this->port);
    if ($bind == false)
    {
      $err = @socket_strerror(@socket_last_error($this->server_handle));
      @socket_close($this->server_handle);
      Log::Error($err);
      exit($err);
    }

    $listen = @socket_listen($this->server_handle, $this->back_log);
    if ($listen == false)
    {
      $err = @socket_strerror(@socket_last_error($this->server_handle));
      @socket_close($this->server_handle);
  ...
  private function service()
  {
    ...
    while ($this->status == self::RUNNING)
    {
      ...
      $socket = @socket_accept($this->server_handle);
      if (!$socket)
        continue;
      ...
      if ($has_err === false)
      {
        if ($this->wrap)
        {
          $message = NetUtil::tcpSocketRead($socket, $this->max_length);
          if ($message === false)
          {
            $has_err = true;
            $err_msg = NetUtil::$errMsg;
          }
        }
        else
        {
          $message = @socket_read($socket, $this->max_length);
          if ($message === false)
          {
            $has_err = true;
